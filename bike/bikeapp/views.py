...
def login(request):
    return render(request,'bikeapp/login2.html')
...
def save(request):
    ...
    if has_register == 0:
        # 将用户名与密码插入到数据库中
        sql2 = 'insert into Users(uid,uname,upassword) values(%s,%s,%s)'
        cursor.execute(sql2, (userID, name, password))
        db.commit()
        ...
        return render(request, 'bikeapp/registershare.html', {'has_register':has_register, 'userName': name})
...
def query(request):
    ...
    if has_user == 1:
     ...
     # return HttpResponse('登录成功')
        ...
        return render(request, 'bikeapp/loginshare.html', {'has_user': has_user})
